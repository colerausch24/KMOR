...
data(iris)
set.seed(123)
# create a new data frame with 50 rows of random values as outliers
# Generate some outliers
ol<-4 #number of outliers
outliers <- data.frame(
  Sepal.Length = rnorm(ol, mean = 10, sd = 1),
  Sepal.Width = rnorm(ol, mean = 10, sd = 1),
  Petal.Length = rnorm(ol, mean = 10, sd = 1),
  Petal.Width = rnorm(ol, mean = 10, sd = 1)
, Species = "outlier")
library(factoextra)

# Extract numerical variables from iris
iris_num <- iris[, c(1:4)]
# Combine iris and outliers
colnames(outliers) <- colnames(iris)
iris_with_outliers <- rbind(iris, outliers)


# Find ideal number of clusters
wssplot(iris_num)
# Run KMOR and regular kmeans on iris_with_outliers
set.seed(123)

kmor_result <- KMOR(iris_with_outliers[, 1:4], 3)
kmeans_result <- kmeans(iris_with_outliers[, 1:4], 3)
kmeans_result_with_added_cluster <- kmeans(iris_with_outliers[, 1:4], 4)
kmeans_iris<-kmeans(iris_num,3)
# Compare the cluster assignments
kmeans_result$size
kmor_result$result$size
...
kmor_result$result$centers
kmeans_result$totss
kmor_result$result$totss
kmeans_result_with_added_cluster$totss
kmeans_result$withinss
kmor_result$result$withinss
kmeans_result_with_added_cluster$withinss
kmeans_result$tot.withinss
kmor_result$result$tot.withinss
kmeans_result_with_added_cluster$tot.withinss
kmeans_result$betweenss
kmor_result$result$betweenss
...
```{r iris 12 outliers further distance}
###########################
#Testing with IRIS
###########################
...
ol<-12 #number of outliers
outliers <- data.frame(
  Sepal.Length = rnorm(ol, mean = 10, sd = 3),
  Sepal.Width = rnorm(ol, mean = 10, sd = 3),
  Petal.Length = rnorm(ol, mean = 10, sd = 3),
  Petal.Width = rnorm(ol, mean = 10, sd = 3)
, Species = "outlier")
library(factoextra)
...
pacoplot(data = iris_with_outliers, clusters = kmeans_result$cluster)
pacoplot(data = kmor_result$new_data, clusters = kmor_result$result$cluster)
pacoplot(data = iris_with_outliers,cluster=kmeans_result_with_added_cluster$cluster)
